...
static dma_cookie_t start_transfer(struct dma_proxy_channel *pchannel_p)
{
	...
	if (!tx_desc) {
		...
		tx_desc->callback_param = &pchannel_p->cmp;
...
static void wait_for_transfer(struct dma_proxy_channel *pchannel_p)
{
	...
	status = dmaengine_tx_status(pchannel_p->channel_p, pchannel_p->cookie, &state);

	/* Regardless of hows we ended update the channel lenth */
	// HACK to get next int32 after the descriptor. (could be cleaner)
       // transferred = *((int*)(((char*)(pchannel_p->tx_desc)) + sizeof(struct dma_async_tx_descriptor)));

	// Subtract residue from requested length to get the transferred length.
	pchannel_p->interface_p->length -= state.residue;
...
static void test(void)
{
	...
	for (i = 0; i < test_size; i++)
		if (channels[0].interface_p->buffer[i] !=
			...
			if (error_count++ > 100) {
				printk("more than 100 buffer errors, aborting check\n");
				break;
			}
		}
}
